...
% Parse Data
S = GEM.S;
N_Original_Rxns = size(S, 2);
RevRxns = find(GEM.rev == 1);
IrrRxns = (setdiff(1:N_Original_Rxns,RevRxns))';

for i = 1:length(D),
    if length(Dstd) > 1,
        if isnan(Dstd(i)) || isinf(Dstd(i)),
           Dstd(i) = 1;
           warning('Data std point Dstd(%d) is NaN or Inf and has been converted to 1',i);
        end
    end
end
...
Sam = [S(:, Irr_D), S(:, Irr_nD),
 S(:, Rev_D), S(:, Rev_nD), -S(:, Rev_D), -S(:, Rev_nD)];
N_Rxns = size(Sam,2);
N_Mets = size(Sam,1);
...
V = zeros(N_Original_Rxns,length(L));
...
if length(rxns_lb) ~= 1,
  for rxn_i = 1:N_Original_Rxns,
      lb_rxns(ismember(Rxns_Or, rxn_i)) = rxns_lb(rxn_i);
  end
end

if length(rxns_ub) ~= 1,
    for rxn_i = 1:N_Original_Rxns,
        ub_rxns(ismember(Rxns_Or, rxn_i)) = rxns_ub(rxn_i);
    end
end
...
lb = [zeros(2 * NIrr_D + 4*NRev_D, 1);
lb_rxns;zeros(N_Rev, 1)];
ub = [repmat(E_max, (2 * NIrr_D + 4 * NRev_D), 1);
ub_rxns;ones(N_Rev, 1)];
...
for i = 1:length(L),
    ...
    V(:, i) = zeros(N_Original_Rxns, 1);
    V(Irr_D, i) = Virr_D;
    V(Irr_nD, i) = Virr_nD;
    V(Rev_D, i) = Vfor_D - Vrev_D;
    V(Rev_nD, i) = Vfor_nD - Vrev_nD;
    V((abs(V(:, i)) < v_eps), i) = 0;
...
if length(Dstd) > 1,
    ...
    Weight(Irr_D) = W(1:NIrr_D);
    Weight(Rev_D) = W(2 * NIrr_D + 1:2 * NIrr_D + NRev_D);
    out.W = Weight;
