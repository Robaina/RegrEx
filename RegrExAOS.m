function out = RegrExAOS(GEM,D,RegrExSol,Options)
...
%     
%     Options.v_eps: scalar indicating the threshold value to consider a
...
tic
    ...
    D = MaxCapacity*D;
    Vopt = RegrExSol.Flux;
%     Vopt=round((1/v_eps)*Vopt)/(1/v_eps);
    S = GEM.S;
    ...
    Irr_D = Irr(~isnan(D(Irr)));
    Irr_nD = Irr(isnan(D(Irr)));
    Rev_D = Rev(~isnan(D(Rev)));
    Rev_nD =  Rev(isnan(D(Rev)));
   ...
   for l = 1:Nsamples,
    ...
    while j <= samplesize,
        ...
        if ~isempty(tasks),
           Vss1min((ismember(Rxns_Or,tasks))) = v_eps;
        end
        if ~isempty(blocked),
           Vss1max((ismember(Rxns_Or,blocked))) = v_eps;
        end
...
%         Vss2max(1:NIrr)=0;
        ...
        vecsense = [repmat('=',Mets,1);repmat('=',NIrr+NRev,1);repmat('<',2*NRev,1);repmat('=',NRev+NIrr_D+2*NRev_D,1);repmat('=',2,1)];
        b = [zeros(Mets,1);VpIrr;zeros(NRev,1);zeros(NRev,1);Vss2max(NIrr+1:Rxns);VpRev;DIrr;zeros(NRev_D,1);DRev;ZE;ZV];
        ...
        %Construction of constraints matrix Amat:  
        ...
        Amat = [A1;A4;A5;A6;A7;A8;A9;A10;A11;A16;A17];
...
%         params.SubMIPNodes
        ...
        try
            ...
            %Recover flux distribution from gurobi output
            VIrrD = X(1:NIrr_D);
    ...
    SolVT = [SolVT,SolV];SolVT(abs(SolVT) < v_eps) = 0;
